...
function interrogateStep(step) { // method used to parse a given test step and perform the configured actions
    ...
    page.evaluate(function(step) {
        ...
        if ( step.actions != null ) {
            step.actions.map(function(action) {
                ...
                return performAction(action); // method available in the injected commands.js file.
            });
        }
    }, step); // pass "step" as an argument to the evaluate method
}
    ...
    if ( testcase != null && !bHasTestCase ) { // condition when we have first loaded the external testcase json file
        ...
        if ( current_step < max_step ) {
            ...
            if (page.injectJs('commands.js')) { // Inject our Commands so we can perform actions configured in testcase.json file
                interrogateStep(current_step_info);
            }
        } else {
            phantom.exit();
        }
